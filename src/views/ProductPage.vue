...
// ✅ Personaliseer opties (alleen voor aanpasbare kussens)
const selectedFirstSide = ref('');
const selectedSecondSide = ref('');
const selectedFilling = ref('');

// ✅ Toevoegen van productbeschrijving voor niet-aanpasbare kussens
...
<template>
  <div v-if="product" class="bg-[#E5D9CF] min-h-screen font-sans p-6">
    ...
    <div class="max-w-5xl mx-auto bg-white p-6 shadow-lg rounded-lg flex flex-col lg:flex-row gap-8">
      ...
      <div class="w-full lg:w-1/2 flex flex-col items-center">
        <img :src="getImageUrl(product.imageUrl)" alt="Product Image" class="rounded-lg shadow-md w-full object-cover" />
      </div>
      ...
      <div class="w-full lg:w-1/2">
        ...
        <p v-if="product.category !== 'customizable'" class="mt-4 text-gray-600">
          {{ extraDescription }}
        </p>

        <!-- ✅ "Add to Cart" Button -->
        <button class="mt-6 w-full bg-[#A3C7D6] text-white px-6 py-3 rounded-lg hover:bg-[#89A5B6] transition duration-300">
          In winkelwagen
        </button>
      </div>
    </div>
